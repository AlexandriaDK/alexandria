...
{if isset($user_editor)}
  ...
  {/if}
  {if ! $id && $type == 'tag'}
    <a href="adm/tag.php?tag={$tag|rawurlencode}" accesskey="r" title="Hotkey: R"><span
        class="updatelinktext">{$_update_edit}</span></a>
  {elseif $type != 'awards'}
    <a href="adm/redir.php?cat={$type}&amp;data_id={$id}" accesskey="r" title="Hotkey: R"><span
        class="updatelinktext">{$_update_edit}</span></a>
  {/if}
{/if}
