    ...
    {if $pic}
      <div style="float: right;">
        <a href="/gfx/tag/l_{$id}.jpg">
          <img src="/gfx/tag/s_{$id}.jpg" alt="{$tag|escape}" title="{$tag|escape}"
    ...
    <table class="indata">
      {foreach $slist as $game}
        <tr>
          ...
          <td {if $game.cancelled}class="cancelled" {/if}>{if isset($game.conlink)}<a href="{$game.conlink}" class="con"
            title="{$game.coninfo}">{$game.conname|escape}</a>{/if}</td>
      </tr>
    {/foreach}
  </table>
  {/if}
