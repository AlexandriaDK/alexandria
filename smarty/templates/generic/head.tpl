    ...
    {if ! isset($dberror) && ! isset($installation) }
      ...
      {if isset($URLLANG) }
        <div class="leftmenucontent selectlanguage">
          ...
          {$_chooselanguage}
          <br><br>
          {foreach $ALEXLANGUAGES as $altlanguage => $altlanguagelocalname}<a href="/{$altlanguage}/{$BASEURI}"
              hreflang="{$altlanguage}"
              title="{$altlanguagelocalname|escape}">{$altlanguage}</a>{if not $altlanguagelocalname@last} • {/if}{/foreach}
          </div>
        {/if}
