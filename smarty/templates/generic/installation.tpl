...
<div id="content" lang="en">
  ...
  {if $stage == 'dbsetup'}
    ...
    <p>
      Import structure and data from Alexandria.dk? If you have any old Alexandria tables in the selected database <code
        class="label">{$dbname|escape}</code> they will be deleted.
    </p>
  ...
  {elseif $stage == 'populate'}
    ...
    <p>
      Import content from Alexandria.dk? If you have any content in the tables in the selected database <code
        class="label">{$dbname|escape}</code> it will be deleted.
    </p>
    <form action="./" method="post">
      <div>
        <input type="hidden" name="token" value="{$token}">
  ...
  {elseif $stage == 'ready'}
    ...
    <p>
      Data has been successfully imported. Active the site? This is a local action and does not contact or notify any
      external services.
    </p>
    <form action="./" method="post">
      <div>
  ...
  {if $stage != 'ready'}
  <p class="installnote">
    Data is fetched from Alexandria's API at <a href="https://alexandria.dk/export">https://alexandria.dk/export</a>
      (JSON format). <span id="checkserver">Checking if service is online...</span>
    </p>
  {/if}
</div>
<script>
