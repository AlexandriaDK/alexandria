...
<div id="content">
  ...
  {if $pic}
    <div class="thumb">
      <a href="/gfx/scenarie/l_{$id}.jpg">
        <img src="/gfx/scenarie/s_{$id}.jpg" alt="{$_sce_frontpagefor} {$title|escape}"
          title="{$_sce_frontpagefor} {$title|escape}">
      </a>
    </div>
  {/if}
  ...
  {if $sysstring != "" || $genre != "" || $participants != "" || $tags || isset($user_id)}
    {if isset($user_id)}
      ...
      <ul class="taglist">
          ...
          <li>
            {if isset($user_can_edit_tag[$tag_id]) && $user_can_edit_tag[$tag_id] }
              <span id="tagdelete_{$tag_id}" class="delete">
                <a href="adm/user_tags.php?scenario={$id}&tag_id={$tag_id}&action=delete&token={$token}"
                  title="{$_sce_removetag|escape}">[{$_remove|ucfirst|escape}]</a></span>
            {/if}
        ...
        {if isset($user_id)}
          <li><a href="#" onclick="$('#tag_input_li').toggle(100).focus();$('#tag_input').focus();" title="{$_sce_addtag}"
              accesskey="t" class="tag">+</a></li>
          <li style="display: none;" id="tag_input_li"><input type="hidden" name="scenario" value="{$id}"><input
              type="hidden" name="action" value="add"><input type="text" name="tag" id="tag_input" class="newtag"
              placeholder="E.g. Grind Night"></li>
        {/if}
      </ul>
      {if isset($user_id)}
      </form>
    ...
    <p class="indata">
      ...
      {if isset($user_id) && $participants == ""}
        {$_participants|ucfirst}: {$_unknown|ucfirst}. <a href="#"
          onclick="document.getElementById('form_participants').style.display='block'; return false;">{$_sce_addplayerno}</a>
      {/if}
    </p>
    {if isset($user_id) }
      <div id="form_participants" style="display: none">
        <form action="adm/user_participants.php" method="post">
          ...
          <table>
            <tr>
              <td>{$_sce_nogms}:</td>
              <td><input type="text" name="gms" size="2" value="{$gms}" /></td>
            </tr>
            <tr>
              <td>{$_sce_noplayers}:</td>
              <td><input type="text" name="players" size="2" value="{$players}" /></td>
            </tr>
            <tr>
              <td></td>
              <td><input type="hidden" name="scenarie" value="{$id}" /><input type="submit"
                  value="{$_edit|ucfirst|escape}" /></td>
            </tr>
          </table>
  ...
  {if count($descriptions) gt 1}
    ...
    {if $descriptions}
        ...
        <div id="tabs" style="margin-top: 0">
          <ul>
            {foreach $descriptions AS $d_id => $d}
              <li><a href="#description-{$d_id}" {if isset($d.langname)}title="{$d.langname|escape}"
                    {/if}>{$d.language|escape}{if $d.note} ({$d.note}){/if}</a></li>
              {/foreach}
            </ul>
      ...
      {if $runlist != ""}
        <h3 class="parttitle">
          {if $conlist == ""}{$_sce_runs}{else}{$_sce_furtherruns}{/if}{if $haslocationscount > 0} <a
            href="locations?game_id={$id}"
            title="{$haslocationscount} {if $haslocationscount == 1}{$_location}{else}{$_locations}{/if}">🗺️</a>{/if}
        </h3>
        {$runlist}
      {/if}
