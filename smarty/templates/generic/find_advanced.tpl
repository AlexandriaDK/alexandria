  ...
  <form action="find">
    <table>
      ...
      <tr>
        ...
        <td style="column-count: 2;">
          {section name=g loop=$genres}
            <input type="checkbox" name="search_genre[]" value="{$genres[g].id}" id="search_genre_{$genres[g].id}"><label
              for="search_genre_{$genres[g].id}">{$genres[g].name}</label>{if ! $smarty.section.g.last}<br>{/if}
      ...
      <tr>
        ...
        <td style="column-count: 2;">
          {section name=c loop=$categories}
            <input type="checkbox" name="search_genre[]" value="{$categories[c].id}"
              id="search_genre_{$categories[c].id}"><label
              for="search_genre_{$categories[c].id}">{$categories[c].name}</label>{if ! $smarty.section.c.last}<br>{/if}
      ...
      <tr>
        ...
        <td><select name="search_filelanguage[]" size="{$suggestlanguages|@count}"
            multiple>{foreach $suggestlanguages as $language}<option value="{$language.code|escape}">
                {$language.name|escape} ({$language.count} {if $language.count == 1}{$_game}{else}{$_games}{/if})</option>
            {/foreach}</select>
      </tr>

      <tr>
        <td></td>
        <td><input type="hidden" name="search_type" value="findspec"><input type="submit" value="{$_search|escape}">
        </td>
      </tr>
