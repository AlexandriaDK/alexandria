...
{/section}
				...
				<tr>
					...
					<td><select name="search_filelanguage[]" size="{$suggestlanguages|@count}" multiple>{foreach $suggestlanguages as $language}<option value="{$language.code|escape}">{$language.name|escape} ({$language.count} {if $language.count == 1}{$_game}{else}{$_games}{/if})</option>{/foreach}</select>
				</tr>
