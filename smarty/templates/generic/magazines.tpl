{assign var="pagetitle" value="{if $issueid}{$issue.magazinename} - {$issue.title} -
{if $issue.releasetext}{$issue.releasetext}{/if}{elseif $magazinename}{$magazinename}{else}{$_magazines_title}{/if}"}
    ...
    {if $issueid}
      ...
      <div class="arrows">
        {if $arrowset.prev.active}
          <a href="magazines?issue={$arrowset.prev.id}"
            title="{$arrowset.prev.title|escape}{if $arrowset.prev.releasetext} - {$arrowset.prev.releasetext|escape}{/if}"
            rel="prev">←</a>
        {else}
