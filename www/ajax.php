...
if (strlen($term) >= 2) {
	$query = "
		...
		UNION ALL
			SELECT convention.id, CONCAT(convention.name,' (',COALESCE(convention.year,'?'),')') AS label, 'convention' AS type, 'con' AS linkpart, 'convent' AS filepath, '' AS note FROM convention
