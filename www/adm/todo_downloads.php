...
if ($action == "calculate") {
	...
	foreach ($authors AS $author) {
		$aid = $author['id'];
		$scenarios = getcol("SELECT game_id FROM pgrel LEFT JOIN files ON pgrel.game_id = files.data_id AND files.category = 'sce' WHERE files.id IS NULL AND pgrel.title_id = 1 AND pgrel.person_id = $aid");
