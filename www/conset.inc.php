...
if ($r['id'] == 0) {
	...
	if ($_SESSION['user_id']) {
		...
		$q = getall("
			SELECT convent.id, convent.name, convent.begin, convent.end, convent.year, convent.place, convent.cancelled, userlog.type IS NOT NULL AS visited, COALESCE(convent.country, conset.country) AS country
			FROM convent 
