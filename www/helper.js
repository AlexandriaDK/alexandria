    ...
    .autocomplete({
      ...
      select: function (event, ui) {
        if (ui.item.linkpart == "magazine") {
          ...
          window.location =
            "data?" + ui.item.linkpart + "=" + encodeURIComponent(ui.item.id);
        }
      },
    })
...
function selectAwardYear(year) {
  if (
    document
      .querySelector("[data-year='" + year + "'].yearselector")
      .getAttribute("data-selected") == 1
  ) {
    // snow all
    document
      .querySelectorAll("[data-year].awardyear")
      .forEach(function (value) {
        value.style.display = "block";
      });
    document
      .querySelectorAll("[data-year].yearselector")
      .forEach(function (value) {
    ...
    // show specific
    document
      .querySelectorAll(":not([data-year='" + year + "']).awardyear")
      .forEach(function (value) {
        value.style.display = "none";
      });
    document
      .querySelectorAll(":not([data-year='" + year + "']).yearselector")
      .forEach(function (value) {
    ...
    document
      .querySelectorAll(":not([data-year='" + year + "']).yearselector")
      .forEach(function (value) {
        value.setAttribute("data-selected", 0);
      });
    document
      .querySelectorAll("[data-year='" + year + "'].awardyear")
      .forEach(function (value) {
        value.style.display = "block";
      });
    document.querySelector(
      "[data-year='" + year + "'].yearselector"
    ).style.opacity = 1;
    document
      .querySelector("[data-year='" + year + "'].yearselector")
...
function selectAwardCategory(category) {
  if (
    document.querySelector(
      "[data-category='" + category + "'].categoryselector"
    ).dataset.selected == 1
  ) {
    // snow all
    document
      .querySelectorAll("[data-category].awardcategory")
      .forEach(function (value) {
        value.style.display = "block";
      });
    document
      .querySelectorAll("[data-category].categoryselector")
      .forEach(function (value) {
        value.style.opacity = 1;
      });
    document.querySelector(
    ...
    document
      .querySelectorAll(
        ":not([data-category='" + category + "']).awardcategory"
      )
      .forEach(function (value) {
        value.style.display = "none";
      });
    document
      .querySelectorAll(
        ":not([data-category='" + category + "']).categoryselector"
      )
      .forEach(function (value) {
        value.style.opacity = 0.5;
      });
    document
      ...
      .forEach(function (value) {
        value.dataset.selected = 0;
      });
    document
      .querySelectorAll("[data-category='" + category + "'].awardcategory")
      .forEach(function (value) {
        value.style.display = "block";
      });
    document.querySelector(
...
function selectAwardOption(value, key) {
  if (
    document.querySelector(
      "[data-" + key + "='" + value + "']." + key + "selector"
    ).dataset.selected == 1
  ) {
    // snow all
    document
      .querySelectorAll("[data-" + key + "].award" + key + "")
      .forEach(function (value) {
        value.style.display = "block";
      });
    document
      .querySelectorAll("[data-" + key + "]." + key + "selector")
      .forEach(function (value) {
        value.style.opacity = 1;
      });
    document.querySelector(
    ...
    document
      .querySelectorAll(
        ":not([data-" + key + "='" + value + "'])." + key + "selector"
      )
      .forEach(function (value) {
        value.style.opacity = 0.5;
      });
    ...
    document
      .querySelectorAll("[data-" + key + "='" + value + "'].award" + key + "")
      .forEach(function (value) {
        value.style.display = "block";
      });
    document.querySelector(
    ...
    $(".awardyear").each(function () {
      let visibleData = $(this).children("div").children("div").is(":visible");
      if (!visibleData) {
        $(this).hide();
      }
    });
...
function refreshOption(key) {
  ...
  $("." + classname).each(function () {
    ...
    if (data.length == 0 || data.indexOf(dvalue) != -1) {
      // present or none selected
      $(this).css("opacity", 1);
    } else {
  ...
  $(".awardyear").each(function () {
    ...
    if (awardYears.length == 0 || awardYears.indexOf(dvalue) != -1) {
      // present or none selected
      $(this).show();
    } else {
      $(this).hide();
...
function toggleConCountries(countryCode) {
  ...
  $(".conblock ul li").show();
  if (countryCode) {
    $(".conblock[data-countries]")
      .not('[data-countries~="' + countryCode + '"]')
      .hide();
    $(".conblock ul li[data-country]")
      .not('[data-country="' + countryCode + '"]')
      .hide();
  }
}
...
cheet("↑ ↑ ↓ ↓ ← → ← → b a", function () {
  location.href =
    "https://alexandria.dk/myhistory?achievement=upupdowndownleftrightleftrightba";
});
